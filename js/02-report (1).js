...
function evalReflection(n){
  ...
  if(isLazy){
    ...
    icon = pts>=6?'🌟':pts>=4?'👍':'🙂';
    msg = pts>=6?'Wonderful reflection! That is a real achievement!':pts>=4?'Good effort! Being specific helps you remember your wins.':'Try to be more specific — what exactly did you do?';
    ...
    pts = isGenuine ? (words>=12?7:words>=7?5:3) : 2;
    icon = pts>=6?'💪':pts>=4?'👍':'🙂';
    msg = pts>=6?'Excellent self-awareness! Knowing what to improve is the first step!':pts>=4?'Good honest reflection! The more specific, the better.':'Be more honest and specific — what exactly needs improving?';
    ...
    pts = isGenuine ? (words>=12?7:words>=7?5:3) : 2;
    icon = pts>=6?'🎯':pts>=4?'👍':'🙂';
    msg = pts>=6?'Great plan! A clear goal for tomorrow sets you up for success!':pts>=4?'Good plan! Try to make it even more specific next time.':'Make your plan more actionable — say exactly what you will do!';
...
async function approveReport(){
  ...
  if(appMsg)appMsg.textContent='✅ +'+pts+' points approved! Keep shining! 🌟';
  ...
  populateResetDatePicker();
  showPop('✅','Report Approved! 🎉','your child your report has been approved! You earned '+pts+' points today! ⭐ Keep it up!');
  ...
  if([5,10,15,20,30].includes(streak)){
    ...
    setTimeout(()=>showPop('🔥',streak+'-Day Streak!','Amazing consistency! +'+bonus+' bonus points!'),1800);
  }
}

async function rejectReport(){
  const date = _pendingReviewDate || document.getElementById('rpt-date')?.value;
  if(!confirm('REJECT this report? This will completely erase all entries for '+(date||'today')+'. your child will need to start fresh.')) return;
  ...
  showPop('❌','Report Rejected!','The report has been completely erased. your child, please fill everything carefully and resubmit for approval! 💪');
...
// ════════════════════════════════════════════
const VAANYA_QUOTES = [
  {icon:'\u{1F31F}',title:"You're a Superstar!",msg:"Every point you earn is a step closer to your dreams. Keep shining, beta!"},
  {icon:'\u{1F525}',title:"On Fire Today!",msg:"Look at those points adding up! Consistency is your superpower!"},
  ...
  {icon:'\u{1F308}',title:"Brilliant!",msg:"Hard work today = amazing rewards tomorrow. You are building an incredible future!"},
  {icon:'\u{1F451}',title:"The Star of the House!",msg:"Every effort you make today makes tomorrow better. You ARE amazing!"},
  {icon:'\u{1F48E}',title:"Diamonds are Made Under Pressure!",msg:"Tough tasks make you stronger. You are becoming unstoppable!"},
...
const SPEND_QUOTES = [
  ...
  {q:"When you earn it yourself, even a simple sweet tastes like a victory!", attr:"— your child's motto", grad:"linear-gradient(135deg,#FF9A3C,#FF006E)", emoji:"🍬"},
...
function renderParentTab(){
  ...
  if(shlokaRef){
    shlokaRef.textContent = shlokaReflect.length>2 ? shlokaReflect : '— your child has not filled this yet —';
